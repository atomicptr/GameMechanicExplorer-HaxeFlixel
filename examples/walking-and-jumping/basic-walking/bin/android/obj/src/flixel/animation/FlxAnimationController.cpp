		...
		HX_STACK_LINE(556)
		{
			...
			while((true)){
				...
				if ((this->_sprite->framesData->framesHash->exists(name))){
					...
					HX_STACK_LINE(562)
					int _g1 = ::flixel::util::FlxArrayUtil_obj::indexOf_flixel_system_layer_frames_FlxFrame(this->_sprite->framesData->frames,frameToAdd,null());		HX_STACK_VAR(_g1,"_g1");
					HX_STACK_LINE(562)
					AddTo->push(_g1);
				}
			}
...
Void FlxAnimationController_obj::byPrefixHelper( Array< int > AddTo,Array< ::Dynamic > AnimFrames,::String Prefix){
{
		...
		HX_STACK_LINE(613)
		{
			...
			while((true)){
				...
				int i = (_g)++;		HX_STACK_VAR(i,"i");
				HX_STACK_LINE(615)
				int _g1 = ::flixel::util::FlxArrayUtil_obj::indexOf_flixel_system_layer_frames_FlxFrame(this->_sprite->framesData->frames,AnimFrames->__get(i).StaticCast< ::flixel::system::layer::frames::FlxFrame >(),null());		HX_STACK_VAR(_g1,"_g1");
				HX_STACK_LINE(615)
				AddTo->push(_g1);
			}
		}
...
::String FlxAnimationController_obj::set_frameName( ::String Value){
	...
	if (((  (((this->_sprite->framesData != null()))) ? bool(this->_sprite->framesData->framesHash->exists(Value)) : bool(false) ))){
		...
		if (((frame != null()))){
			HX_STACK_LINE(668)
			int _g = ::flixel::util::FlxArrayUtil_obj::indexOf_flixel_system_layer_frames_FlxFrame(this->_sprite->framesData->frames,frame,null());		HX_STACK_VAR(_g,"_g");
			HX_STACK_LINE(668)
			this->set_frameIndex(_g);
		}
	}
