...
Void Lib_obj::rethrow( Dynamic error){
{
		...
		if ((!(event->__getIsCancelled()))){
			...
			HX_STACK_LINE(300)
			::haxe::Log_obj::trace(message,hx::SourceInfo(HX_CSTRING("Lib.hx"),300,HX_CSTRING("flash.Lib"),HX_CSTRING("rethrow")));
			HX_STACK_LINE(304)
			::Sys_obj::exit((int)1);
		}
	}
...
Void Lib_obj::exit( ){
{
		...
		if (((quit != null()))){
			HX_STACK_LINE(389)
			if (((quit == ::flash::Lib_obj::close_dyn()))){
				HX_STACK_LINE(391)
				::Sys_obj::exit((int)0);
			}
			HX_STACK_LINE(396)
...
{
		HX_STACK_FRAME("flash.Lib","postUICallback",0x4a410bc0,"flash.Lib.postUICallback","flash/Lib.hx",435,0x26d02e78)
		HX_STACK_ARG(inCallback,"inCallback")
		HX_STACK_LINE(435)
		::flash::Lib_obj::lime_post_ui_callback(inCallback);
	}
return null();
}
...
void Lib_obj::__boot()
{
	...
	__moduleNames= null();
	__stage= null();
	lime_post_ui_callback= ::flash::Lib_obj::load(HX_CSTRING("lime"),HX_CSTRING("lime_post_ui_callback"),(int)1);
